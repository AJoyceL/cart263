window.onload = function () {
  ...
  function animateSquirrel() {
    ...
    // check collisions each frame so nuts vanish when collected
    collisionDetection();
  ...
  function collisionDetection() {
    //collision detection between squirrels and temp nuts
    for (let i = 0; i < garden.squirrels.length; i++) {
      for (let j = 0; j < garden.tempNuts.length; j++) {
        garden.squirrels[i].catchNut(garden.tempNuts[j])
      }
    }
  }
